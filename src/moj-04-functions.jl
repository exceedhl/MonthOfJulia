...
#
function rectangle(x, y = x; color = "black", θ = pi / 2) # Can use , rather than ;
...
#
(sin*sin)(pi/4) == sin(sin(pi/4))                 # Works as a result of function composition with *
...
function sametype(x::T, y::T) where {T}
...
#
function samereal(x::T, y::T) where {T<:Real}
...
@show code_lowered(divide, (Int, Int))

@show code_typed(divide, (Int, Int))

# Bytecode generated by the LLVM JIT compiler.
#
@show code_llvm(sqrt, (Float64,))
@show code_llvm(divide, (Int, Int))
code_llvm(square, (Int64,))

# Native assembly code.
#
@show code_native(divide, (Int, Int))
